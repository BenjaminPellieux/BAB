...
*/
static void get_settings_ctx(settings_context *settings_ctx)
{
...
static signed char check_eeprom_save_ctx(settings_context* settings_ctx){
  int8_t err;
  Serial.println("Verifying save");
  int16_t address = 1;
  if (settings_ctx->mode_jauge_smiley != EEPROM.readByte(address)) {
    err = -UNVALID_CHECK_SAVE;
  }
  //The smallest addressable "thing" in C is a char. Every variable in C must have a unique address, 
  //therefore your bool can't be smaller than that. (Note that char isn't always 8 bits though)
  
  address += sizeof(int8_t); // 
  if (settings_ctx->seuil_1 != EEPROM.readUShort(address)) {
    err = -UNVALID_CHECK_SAVE;
  }
  address += sizeof(settings_ctx->seuil_1);
  
  if (settings_ctx->seuil_2 != EEPROM.readUShort(address)) {
    err = -UNVALID_CHECK_SAVE;
  }
  address += sizeof(settings_ctx->seuil_2);
  
  if (settings_ctx->brightness != EEPROM.readUShort(address)) {
    err = -UNVALID_CHECK_SAVE;
  }
  address += sizeof(settings_ctx->brightness);
  
  if (settings_ctx->sensitivity != EEPROM.readULong(address)) {
    err = -UNVALID_CHECK_SAVE;
  }
  address += sizeof(settings_ctx->sensitivity);
  
  if (settings_ctx->connect_type != EEPROM.readUShort(address)) {
    err = -UNVALID_CHECK_SAVE;
  }
  
  address += sizeof(settings_ctx->connect_type);
  for (int i = 0; i < MAX_FIELD_SIZE; i++) {
...
*/
static void save_settings_ctx(settings_context *settings_ctx)
{
  ...
  EEPROM.commit();
  //verification
  check_eeprom_save_ctx(settings_ctx);
}
...
static wifi_state_enum init_wifi_connect(settings_context *settings_ctx)
{
  ...
  WiFi.mode(WIFI_STA);
  for (i = 0; (i != 99) && (WiFi.status() != WL_CONNECTED); i++) {
      delay(500);
      Serial.print(".");
  }
...
void get_value_request(String line, settings_context* settings_ctx){
...
static void show_settings_context(String line,settings_context* settings_ctx)
{
...
static  void check_connect_param(uint32_t j, uint8_t old_connect_type,settings_context* settings_ctx){
    ...
    if (client.connected()) {
      ...
      if (client.available()) {             // if there's bytes to read from the client,
        ...
        if (currentLine.startsWith("200")) {
          ...
          Serial.println("\nDEBUG : LINE: "+ line);
          get_value_request(line, settings_ctx); 
          show_settings_context(line, settings_ctx);
          check_connect_param(j, old_connect_type, settings_ctx);
